...
import { enrichAndSort, formatDistance } from "./utils";
...
function renderTable(
  spots: ReturnType<typeof enrichAndSort>,
  locationText: string
) {
  ...
  for (const spot of spots) {
    ...
    row.innerHTML = `
      <td class="results-table__pay">
        ...
        <span class="results-table__sub">${spot.meter_id}</span>
      </td>
      ...
      <td class="results-table__actions">
        ...
        <span class="results-table__sub results-table__actions-spacer" aria-hidden="true">&nbsp;</span>
      </td>
    `;
  ...
  resultsCount.textContent = `${spots.length} spot${spots.length === 1 ? "" : "s"} within 1 km · sorted by current rate`;
  show(tableWrap);
}
...
async function loadParking(
  ...
  try {
    ...
    const sorted = enrichAndSort(meters, coords);
    ...
    renderTable(sorted, locationText);
  } catch (err) {
